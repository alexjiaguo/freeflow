...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    var resolvedContextAnalysisModel: String {
        let trimmed = contextModel.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? Self.defaultContextModel : trimmed
    }
    ...
    init() {
        ...
        let resolvedCtxModel = contextModel.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? Self.defaultContextModel : contextModel
        ...
        self.contextService = AppContextService(
            apiKey: resolvedCtxApiKey,
            baseURL: resolvedCtxBaseURL,
            customContextPrompt: customContextPrompt,
            contextModel: resolvedCtxModel
        )
    ...
    func cleanupBeforeDeallocation() {
        // Keep this for manual cleanup if needed, but logic is moved to deinit
    }
    }
    ...
    private func rebuildContextService() {
        contextService = AppContextService(
            apiKey: resolvedContextAnalysisApiKey,
            baseURL: resolvedContextAnalysisBaseURL,
            customContextPrompt: customContextPrompt,
            contextModel: resolvedContextAnalysisModel
        )
    }
    ...
    func retryTranscription(item: PipelineHistoryItem) {
        ...
        let postProcessingService = PostProcessingService(
            apiKey: resolvedPostProcessingApiKey,
            baseURL: resolvedPostProcessingBaseURL,
            preferredModel: resolvedPostProcessingModel,
    ...
    private func stopAndTranscribe() {
        ...
        let capturedSessionIntent = currentSessionIntent
        isRecording = false
        isTranscribing = true
        ...
        audioRecorder.stopRecording { [weak self] fileURL in
            ...
            self.transcribingIndicatorTask = Task { [weak self] in
                ...
                } catch { /* Cancellation error expected */ }
            }
            ...
            self.transcriptionTask = Task {
                do {
                    let transcriptionService = try TranscriptionService(
                        ...
                        transcriptionModel: self.resolvedTranscriptionModel
                    )
                    ...
                    let appContext: AppContext? = await { [weak self] () async -> AppContext? in
                        guard contextGatheringEnabled else { return nil }
                        if let sessionContext {
                            return sessionContext
                        } else if let ctx = await self?.inFlightContextTask?.value {
                            return ctx
                        } else {
                    ...
                    let finalContext = appContext ?? AppContext(
                        appName: nil,
                        bundleIdentifier: nil,
                        windowTitle: nil,
                        selectedText: nil,
                        ...
                        screenshotError: "Context not available"
                    )
                    ...
                    await MainActor.run { [weak self] in
                        ...
                        self.lastContextScreenshotDataURL = finalContext.screenshotDataURL
                        self.lastContextScreenshotStatus = finalContext.screenshotError
                            ?? "available (\(finalContext.screenshotMimeType ?? "image"))"
